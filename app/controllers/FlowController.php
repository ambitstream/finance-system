	...
	public function statistic()
    {
		...
		foreach ($periods as $period) {
				...
				$period_average = $this->db->exec('
					SELECT b.period_id as period_id, a.id as category_id, SUM(b.sum) / '.$period_duration.' as `category_average`
				FROM category as a
				LEFT OUTER JOIN flow as b
				ON (a.id = b.category_id AND b.type = 0 AND  b.period_id = '.($period_id).' 
				...
				$period_monthly_average = $this->db->exec('
					SELECT a.period_id as period_id, SUM(a.sum) / '.$period_duration.' as value
											FROM  flow a 
											Join category b
											ON (a.category_id = b.id)
											WHERE a.type = 0 AND  a.period_id = '.($period_id).' AND b.reporting_type != "yearly" 
											AND a.ignore_in_reports != 1;
				');
						
				// select everything from period and divide sum on the duration
				$period_total_average = $this->db->exec('
				SELECT period_id as period_id, SUM(sum) / '.$period_duration.' as value
							FROM  flow 
							WHERE type = 0 AND  period_id = '.($period_id).'
		...
		//per week
		$weekly_categories_titles = $this->db->exec('
			SELECT id, name
				FROM category
				WHERE reporting_type = "weekly";	
		');
		$this->f3->set('weekly_categories_titles', $weekly_categories_titles);



		
		// period_id = 9 - исправить
		$weekly_categories_weeks = $this->db->exec('
				SELECT yearweek(a.date,1) as `week`
					FROM flow a
					JOIN category b
					ON (a.category_id = b.id)
					WHERE a.period_id = 9 AND b.reporting_type = "weekly"
					Group By week
					ORDER BY week;
		');
		$this->f3->set('weekly_categories_weeks', $weekly_categories_weeks);
		
		
		
		// $weekly_categories_data = new Array();
		$this->f3->set('weekly_categories_data',array());

		
		// period_id = 9 - исправить
		foreach ($weekly_categories_weeks as $key=>$week_number) {
			$weekly_categories_data[$key] = $this->db->exec('
					SELECT  yearweek(date,1) as `week`, a.id as cat_id, SUM(b.sum) as `sum`
										FROM category as a
										LEFT JOIN flow as b
										ON (a.id = b.category_id AND  b.period_id = 9 AND b.ignore_in_reports != 1 AND a.reporting_type ="weekly" )

										AND yearweek(date,1) = '.$week_number["week"].'
										GROUP BY `week` , a.id
										ORDER BY `week`, a.id;
			');
		}
		$this->f3->push('weekly_categories_data',$weekly_categories_data);
		...
		$monthquery = $this->db->exec('
			SELECT TIMESTAMPDIFF(MONTH,MIN(`date`),MAX(`date`)) as period_dur
			FROM flow;
		');
		
		$monthscount = 1;
		$monthscount = 1+$monthquery[0]['period_dur'];
		
		$this->f3->set('averoutpermonth',$this->db->exec('		
			select SUM(`sum`) / ('.$monthscount.') as `averbymonth`
			from `flow`
			WHERE `type` = 0;
		'));
